...
public class OrgController {
    ...
    public Result<List<Organization>> list() {
        return Result.success(orgLogic.listOrgs());
    }

    @GetMapping("/tree")
    public Result<List<Map<String, Object>>> tree() {
        return Result.success(orgLogic.getOrgTree());
    }

    @GetMapping("/{id}")
    public Result<Organization> getById(@PathVariable Long id) {
        return Result.success(orgLogic.getOrgById(id));
    }
